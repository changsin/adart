...
# @router.get("/", response_model=List[schemas.Project1])
@router.get("/", response_model=schemas.Project1WithCount)
def read_project(
    ...
    if crud.user.is_admin(current_user) or crud.user.is_reviewer(current_user):
        count, projects = crud.project1.get_multi_order_by_created_at(
            db,
            name=name,
        ...
        count, projects = crud.project1.get_multi_by_email(
            db,
            is_dir_null=is_dir_null,
        ...
        count, projects = crud.project1.get_multi_by_task_owner(
            db=db,
    ...
    payloads = schemas.Project1WithCount(num_count=count, projects=projects)
    return payloads


@router.post("/", response_model=schemas.Project1)
def create_project(
    *,
    db: Session = Depends(deps.get_db),
    project_total_in: schemas.Project1CreateTotal,
    current_user: models.User = Depends(deps.get_current_active_user),
) -> Any:
    """
    Create new project and related cutomer, annotation_classes, annotation_errors.
    """
    project_in = schemas.Project1Create(**project_total_in.__dict__)
    ...
    project = crud.project1.create_with_annotation_errors_and_classes(
        db=db,
        obj_in=project_in,
...
@router.put("/{id}", response_model=schemas.Project1)
def update_project(
    *,
    db: Session = Depends(deps.get_db),
    id: int,
    project_in: schemas.Project1UpdateTotal,
    current_user: models.User = Depends(deps.get_current_active_user),
) -> Any:
    """
    Update an project.
    """
    project = crud.project1.get(db=db, id=id)
    if not project:
        raise HTTPException(status_code=404, detail="Can't find the project.")
    if not crud.user.is_admin(current_user):
    ...
    project = crud.project1.update_with_annotation_errors_and_classes(
        db=db,
        db_obj=project,
        obj_in=project_in,
        anno_errors=annotation_error_data,
...
@router.get("/{id}", response_model=schemas.Project1Detail)
def read_project(
    *,
    db: Session = Depends(deps.get_db),
    id: int,
    ...
    Get project by ID.
    """
    project = crud.project1.get_with_string_classes(db=db, id=id)
    if not project:
        raise HTTPException(status_code=404, detail="프로젝트를 찾을 수 없습니다.")
    return project


@router.delete("/{id}", response_model=schemas.Project1)
def delete_project(
    *,
    db: Session = Depends(deps.get_db),
    id: int,
    ...
    crud.project1.remove(db=db, id=id)
    return project


@router.get("/create_tasks/{id}", response_model=schemas.Project1)
def create_tasks(
    *,
    db: Session = Depends(deps.get_db),
    id: int,
